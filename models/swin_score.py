...
class CondSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7,
        ...
        self.norm1 = ConditionalInstanceNorm2dPlus(dim, style_dim)
        ...
        self.norm2 = ConditionalInstanceNorm2dPlus(dim, 10)
    ...
    def forward(self, x, y):
        ...
        # Double Attn
        shortcut = x
        x = self.norm1(x.transpose(-1, -2), y).transpose(-1, -2)
        ...
        # FFN
        x = shortcut + x
        x = x + self.mlp(self.norm2(x.transpose(-1, -2), y).transpose(-1, -2))
...
class StyleBasicLayer(nn.Module):
    ...
    def forward(self, x, latent1, latent2, y):
...
class BilinearUpsample(nn.Module):
    ...
    def __init__(self, input_resolution, dim, out_dim=None):
        ...
        self.norm = ConditionalInstanceNorm2dPlus(dim,10)
...
class Swin_Score(nn.Module):
    def __init__(self, config):
        ...
        style_dim = config.model.num_classes
        n_mlp = 8
    ...
    def forward(self, x, y):
        ...
        for layer, to_rgb in zip(self.layers, self.to_rgbs):
            x = layer(x, latent[:,count,:], latent[:,count+1,:], y)
            b, n, c = x.shape
